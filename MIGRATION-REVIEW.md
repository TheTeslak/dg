...
ToC впоследствии был повторно сверен с `TableOfContents.vue`: первая нативная версия оказалась упрощённым редизайном, а не переносом. Восстановлены разорванный вертикальный stepper, иерархия H2/H3, зона раскрытия 240 px, центрирование активного шага, исходные размеры/цвета/переходы и мобильный bottom sheet с анимацией закрытия. H4 больше не попадают в боковой ToC, а `tocAlwaysOn` снова влияет только на вставленный в статью `[[toc]]`, как в `dg`, а не насильно раскрывает боковую навигацию. Для коротких статей с двумя H2/H3 или меньше боковой и мобильный ToC вообще не рендерятся: иначе заголовочный блок выглядит постоянно раскрытым без полезной навигации.
