...
Дата среза: 14 июля 2026 года. Легаси-проект `/home/teslak/Sites/dg/` использован только для чтения. Документ сохраняет найденные риски и отмечает уже закрытые пункты.
...
Исправлено: Astro и RSS используют общие массивы Unified-плагинов из `src/utils/markdown-pipeline.ts`. RSS отличается только конечным семантическим renderer-ом для конструкций, которым в фиде не нужен интерактив. Общие fixtures прогоняют site/feed режимы и запрещают утечку custom tags.
...
Исправлено: home, now и остальные статические страницы находятся в `src/content/pages/{locale}` и проходят строгую схему. Старый `src/data/home.ts`, дубли словарей и неиспользуемые Astro-копии Markdown-компонентов удалены.
...
Текущий `pnpm check` проходит без ошибок, предупреждений и hints. Контент, Markdown fixtures и совпадение i18n-ключей дополнительно проверяются отдельными build-time скриптами.
...
ToC впоследствии был повторно сверен с `TableOfContents.vue`: восстановлены разорванный вертикальный stepper, иерархия H2/H3, зона раскрытия 240 px, центрирование активного шага и мобильный bottom sheet. Как в оригинале, при двух и более заголовках на мобильном отображается нижняя панель с кнопками оглавления и копирования ссылки; safe-area учтён.
...
- [x] Вернуть визуальную и поведенческую эквивалентность бокового ToC.
...
- [x] Один Markdown processor/контракт для HTML и feeds.
- [x] Перенести home/now обратно в типизированный content layer.
- [x] Убрать дубли i18n dictionaries.
- [x] Строгие общие схемы и нормализация данных для Astro/Node.
