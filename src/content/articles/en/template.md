...
lang: en
duration: 5
description: A comprehensive reference for Markdown blocks and custom content components used in this blog's articles.
type: draft
...
A quick copy-paste reference for Markdown blocks and custom content components.
...
Standard Markdown images fit within the content column. Keep the two kinds of image text separate:

- text in `[]` is a useful description for screen readers (`alt`);
- the optional quoted Markdown title becomes the visible caption;
- `| wide` is the only layout modifier and lets the figure break out of the text column on desktop.

There is deliberately no `no-caption` setting. No quoted caption means no `<figcaption>`; a quoted caption means it is shown.

**Standard (with caption):**

```markdown
![A chronological row of illustrated Teslak avatars](/images/avatars.avif "Evolution of the Teslak avatar")
```
...
```markdown
![A chronological row of illustrated Teslak avatars](/images/avatars.avif)
```
...
```markdown
![A chronological row of illustrated Teslak avatars | wide](/images/avatars.avif "Evolution of the Teslak avatar")
```
...
```markdown
![A chronological row of illustrated Teslak avatars | wide](/images/avatars.avif)
```
...
Add `sources: true` to the frontmatter and place `<!-- sources --> ... <!-- /sources -->` markers at the end of the article. Run `pnpm sources` to automatically fetch titles and update the list.
