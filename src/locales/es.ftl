-brand = Teslak

meta-description = Portafolio y Blog de { -brand }

nav-blog = Blog
nav-articles = Artículos
nav-projects = Proyectos
nav-talks = Charlas
nav-streams = Streams
nav-notes = Notas
nav-podcasts = Podcasts
nav-demos = Demos
nav-photos = Fotos
nav-sponsors = Patrocinadores
nav-chat = Hablemos

action-toggle-theme = Alternar tema
action-to-top = Ir arriba
action-back-to-all = Ver todo

blog-only-lang = Mostrar solo { $lang }
blog-nothing-here = { "{" } nada por aquí aún { "}" }
blog-upcoming = Próximamente
blog-draft = Este es un borrador, el contenido puede estar incompleto. Vuelve más tarde.
blog-read-more = Leer más

talks-watch = Ver
talks-transcript = Transcripción
talks-slides = Diapositivas
talks-pdf = PDF
talks-days-left = en { $days } días

projects-subtitle = Proyectos que he creado o mantengo.
projects-recent-releases = Lanzamientos recientes
projects-sort-by-stars = Todos los proyectos por estrellas

sponsors-desc = Si disfrutas de mi trabajo y lo encuentras útil, considera patrocinarme a mí y al ecosistema para ayudar a que el Open Source sea sostenible. ¡Gracias!
sponsors-ecosystem = Patrocinar el ecosistema
sponsors-support-me = Patrocinar a { -brand }
sponsors-afdian = Apoyar en Afdian
sponsors-circles = Círculos de patrocinadores
sponsors-tiers = Niveles de patrocinio
sponsors-how-works = ¿Cómo funciona esto?

footer-copyright = 2026 © { -brand }

intro-desc = Hola, soy { -brand }, un apasionado del código abierto.

page-not-translated = Esta página aún no ha sido traducida.
page-original-version = Versión original en { $lang }.
post-comment-on = comentar en
post-link-telegram = Telegram
post-link-mastodon = Mastodon
post-referenced-by = Referenciado por
post-updated = actualizado
post-draft-notice = Borrador

heading-link = enlace al título
heading-copied = copiado
