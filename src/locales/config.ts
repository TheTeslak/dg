/**
 * Localization architecture (ported from the original `dg` project):
 *
 * - The URL locale controls UI messages, dates, navigation, and document language.
 * - Article content locale comes from its physical `src/content/articles/<locale>` file.
 ...
 * - Missing translations use fallback routes while keeping the requested URL locale.
 * - Only `/` and `/index.html` negotiate a locale from the cookie or Accept-Language.
 *
 * Related implementation:
 * - `negotiation.ts`: initial locale selection
 * - `../utils/i18n.ts`: locale-aware paths and translations
 * - `../utils/article-locales.ts`: article translation and fallback selection
 */
